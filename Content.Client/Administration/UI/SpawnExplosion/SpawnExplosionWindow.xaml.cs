...
public sealed partial class SpawnExplosionWindow : DefaultWindow
{
    ...
    public SpawnExplosionWindow(SpawnExplosionEui eui)
    {
        ...
        _zLevels = _entMan.System<SharedZLevelSystem>();
        _eui = eui;
    ...
    private void SubmitButtonOnOnPressed(ButtonEventArgs args)
    {
        ...
        var explosionType = _explosionTypes[ExplosionOption.SelectedId];
        var cmd = $"explosion {Intensity.Value} {Slope.Value} {MaxIntensity.Value} {MapX.Value} {MapY.Value} {mapId} {explosionType} {GetSelectedWorldZ()}";

        _conHost.ExecuteCommand(cmd);
    }

    private int GetSelectedWorldZ()
    {
        if (_playerManager.LocalEntity is not { } player ||
            !_entMan.TryGetComponent(player, out TransformComponent? transform) ||
            transform.MapID != _mapData[MapOptions.SelectedId])
        {
            return 0;
        }

        return _zLevels.GetWorldZLevel(player);
    }
}
